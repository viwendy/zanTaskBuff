...
class TaskController extends HomeBaseController{
    ...
    public function lists_lb()
    {
        ...
        //供应信息
        $task_list['type_0'] = M('task')->where(array('type'=>0,'status'=>1,'tasklb'=>$level))->field('id,title,level,price,create_time,max_num,apply_num,max_num-apply_num as leftnum, tasklb,task_require,video_url,end_time')->limit(1500)->order('id desc')->select();
        //需求信息
        $task_list['type_1'] = M('task')->where(array('type'=>1,'status'=>1,'tasklb'=>$level))->field('id,title,level,price,create_time,max_num,apply_num,max_num-apply_num as leftnum, tasklb,task_require,video_url,end_time')->limit(1500)->order('id desc')->select();
    ...
    public function submission_task()
    {
        ...
        $list = $model->alias('a')
            ...
            ->where($map)
            ->field('a.*,b.title,b.task_require,b.video_url,b.tasklb,b.level,b.price,b.max_num,b.apply_num,b.max_num-b.apply_num as leftnum,b.level')
