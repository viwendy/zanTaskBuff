...
class MemberController extends HomeBaseController{
    ...
    public function index()
    {
        $data = M('member')->find($this->member_id);
      	$level_info = M('level') -> where(['level'=> $data['level']]) -> find();
      ...
      $where = 'a.member_id ='.$this->member_id.' and a.create_time > '.$start_time.' and a.create_time < '.$end_time;
        ...
        $list = $model->alias('a')
            ...
            ->order('a.id desc')
            ->select();
     	$this -> assign('task_num',count($list));
